...
def smooth(value):
    ...
    try:
        with open("/data/solar.data", "r+") as history:
        ...
        with open("/datatmp/solar_error.log", "a") as log:
            log.write("{}: {}\n".format(time.strftime("%D-%T"), ex))
        data = [0, 0, 0, 0, value]
        with open("/datatmp/solar.data", "w") as history:
            history.write(json.dumps(data))
...
def latest_data(config):
    ...
    try:
        current = format(float(current), '.2f')
    except ValueError:
        current = 0.00001
...
def main():
    ...
    print(latest)
    with open("/data/sunpower", "a") as out:
        out.write(f"{get_ts()},{str(latest)}\n")
